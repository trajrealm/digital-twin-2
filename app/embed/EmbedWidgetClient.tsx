    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center text-center px-6 py-12 h-full">
            <p className="text-2xl font-bold text-gray-900 leading-snug">
              I'm {authorFirstName} {authorLastName}'s digital twin.
            </p>
            <p className="text-lg text-gray-700 mt-3 leading-snug">
              Ask me anything — the real {authorFirstName} {authorLastName} might just respond.
            </p>
            <p className="text-sm text-gray-500 mt-4 max-w-sm leading-relaxed">
              I know {authorFirstName}'s background, courses, and curriculum. I can also put you in touch directly.
            </p>
          </div>
        )}
        {messages.map((msg, idx) => (
          <div
            key={idx}
            className={`flex gap-2 ${msg.role === 'visitor' ? 'justify-end' : 'justify-start'}`}
          >
            {/* Avatar for non-visitor messages */}
            {msg.role !== 'visitor' && (
              <div className="flex flex-col items-center gap-1">
                ...
                <span className="text-xs font-semibold text-gray-600">
                  {msg.role === 'author_live' ? 'Live' : 'Avatar'}
                </span>
              </div>
            )}
            ...
            <div className="flex flex-col">
              {msg.role === 'visitor' && (
                <span className="text-xs font-semibold text-gray-600 mb-1 text-right">You</span>
              )}
              <div
          ...
          <div className="flex justify-start">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 max-w-xs">
              <div className="flex justify-between items-start mb-2">
                <p className="text-sm font-semibold text-gray-900">Ask {authorFirstName} directly</p>
                <button
                  type="button"
                  onClick={() => {
                    setShowEscalation(false);
